const BlockStorage = artifacts.require("./BlockStorage.sol");

require("chai").use(require("chai-as-promised")).should();

contract("BlockStorage", ([deployer, uploader]) => {
  let blockStorage;

  before(async () => {
    blockStorage = await BlockStorage.deployed();
  });

  describe("deployment", async () => {
    it("deploys successfully", async () => {
      const address = await blockStorage.address;
    ...
    it("has a name", async () => {
      const name = await blockStorage.name();
      assert.equal(name, "BlockStorage");
    });
  });
  ...
  describe("file", async () => {
    ...
    before(async () => {
      result = await blockStorage.uploadFile(
        fileHash,
        fileSize,
        fileType,
        fileName,
      ...
      fileCount = await blockStorage.fileCount();
    });
    ...
    //check from Struct
    it("lists file", async () => {
      const file = await blockStorage.files(fileCount);
      assert.equal(
